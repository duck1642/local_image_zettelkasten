...
export type MasonryLayoutEngine = (
  groups: VaultGroup[],
  width: number,
  minWidth?: number,
  ...
  return function computeLayout(
    ...
    if (lastCacheKey === cacheKey && lastCache && lastPositions.length > 0 && groups.length >= lastPositions.length && groups[0].id === lastPositions[0].group.id) {
       for (let i = 0; i < lastPositions.length; i++) {
          ...
          const { height } = estimateMasonryGroupHeight(group, columnWidth, activeIdx, store);

    ...
    try:
        ...
        for source_id in [vault_id_slug(value) for value in source_ids]:
            ...
            try:
                ...
                source_conn.close()
        target_conn.commit()
        # Delete source vaults upon successful merge completion if requested
        if delete_sources:
            for source_id in [vault_id_slug(value) for value in source_ids]:
                try:
                    delete_vault(source_id, confirm=True, ctx=ctx)
                except Exception as exc:
                    try:
                        from logger import log_system
                        log_system("WARNING", "Failed to delete source vault after merge", vault=source_id, error=str(exc))
                    except Exception:
                        pass
    except Exception:
        target_conn.rollback()
        # Revert physical file copies
